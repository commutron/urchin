import React from 'react'


export const BackButton = ({ clearFileThing })=> (
  <button 
    id='doClear'
    title='Clear'
    className='doAction'
    onClick={(e)=>clearFileThing(e)}>
    <svg 
      xmlns="http://www.w3.org/2000/svg"
      height="100%" viewBox="0 0 512 512">
        <path d="M245.09 327.74v-37.32c57.07 0 84.51 13.47 108.58 38.68 5.4 5.65 15 1.32 14.29-6.43-5.45-61.45-34.14-117.09-122.87-117.09v-37.32a8.32 8.32 0 00-14.05-6L146.58 242a8.2 8.2 0 000 11.94L231 333.71a8.32 8.32 0 0014.09-5.97z"/>
        <path fill="none" stroke="currentColor" strokeMiterlimit="10" strokeWidth="32" d="M256 64C150 64 64 150 64 256s86 192 192 192 192-86 192-192S362 64 256 64z"/>
    </svg>
    <span>Try Another</span>
  </button>
);

export const DownloadButton = ({ bID, downFunc })=> (
  <button 
    id={bID}
    title='Download'
    className='doAction'
    onClick={(e)=>downFunc(e)}>
    <svg 
      xmlns="http://www.w3.org/2000/svg"
      height="100%" viewBox="0 0 512 512">
        <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32" d="M336 176h40a40 40 0 0140 40v208a40 40 0 01-40 40H136a40 40 0 01-40-40V216a40 40 0 0140-40h40"/>
        <path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32" d="M176 272l80 80 80-80M256 48v288"/>
    </svg>
  </button>
);

export const PrintButton = ({ pID, printFunc })=> (
  <button 
    id={pID}
    title="Print"
    className='doAction'
    onClick={(e)=>printFunc(e)}>
    <svg 
      xmlns="http://www.w3.org/2000/svg"
      height="100%" viewBox="0 0 512 512">
        <path fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="32" d="M384 368h24a40.12 40.12 0 0040-40V168a40.12 40.12 0 00-40-40H104a40.12 40.12 0 00-40 40v160a40.12 40.12 0 0040 40h24"/>
        <rect width="256" height="208" x="128" y="240" fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="32" rx="24.32" ry="24.32"/><path fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="32" d="M384 128v-24a40.12 40.12 0 00-40-40H168a40.12 40.12 0 00-40 40v24"/>
        <circle cx="392" cy="184" r="24"/>
    </svg>
  </button>
);